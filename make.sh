...
port_os_version=$(echo ${URL} | cut -d"/" -f4)                   # 移植的 OS 版本号, 例: OS1.0.7.0.UNACNXM
port_version=$(echo ${port_os_version} | sed 's/OS1/V816/g')     # 移植的实际版本号, 例: V816.0.7.0.UNACNXM
port_zip_name=$(echo ${URL} | cut -d"/" -f5)                     # 移植的 zip 名称, 例: miui_AURORA_OS1.0.7.0.UNACNXM_81a48e3c20_14.0.zip
...
End_Time() {
  ...
  if [[ $s -ge 10800 ]]; then
    ...
    echo -e "${Blue} - 本次$1用时: $h小时$min分$s秒$ms毫秒"
  elif [[ $s -ge 60 ]]; then
    ms=$(expr $ns / 1000000)
    min=$(expr $s / 60)
    s=$(expr $s % 60)
    echo -e "${Blue} - 本次$1用时: $min分$s秒$ms毫秒"
  elif [[ -n $s ]]; then
    ms=$(expr $ns / 1000000)
    echo -e "${Blue} - 本次$1用时: $s秒$ms毫秒"
  else
    ms=$(expr $ns / 1000000)
    echo -e "${Blue} - 本次$1用时: $ms毫秒"
  fi
}
...
for build_prop in $(sudo find "$GITHUB_WORKSPACE"/images/ -type f -name 'build.prop'); do
  ...
  sudo sed -i 's/'"${port_os_version}"'/'"${vendor_os_version}"'/g' "$build_prop"
  sudo sed -i 's/'"${port_version}"'/'"${vendor_version}"'/g' "$build_prop"
...
for build_prop in $(sudo find "$GITHUB_WORKSPACE"/"${device}"/ -type f -name "*build.prop"); do
  sudo sed -i 's/build.date=[^*]*/build.date='"$build_time"'/' "$build_prop"
  sudo sed -i 's/build.date.utc=[^*]*/build.date.utc='"$build_utc"'/' "$build_prop"
  sudo sed -i 's/ro.mi.os.version.incremental=[^*]*/ro.mi.os.version.incremental='"$port_os_version"'/' "$build_prop"
done
# 精简部分应用
echo -e "${Red} - 精简部分应用"
